...
export default function App() {
  ...
  ): Promise<{ lockedToSacrifice: number; insufficientEvenWithSacrifice: boolean }> => {
    if (!account) return { lockedToSacrifice: 0, insufficientEvenWithSacrifice: false };
  ...
  const handleSavePixels = useCallback(async () => {
    ...
    if (feasibility.insufficientEvenWithSacrifice) {
      showNotification("Not enough PAINT tokens for these pixels, even after freeing your locked ones. Buy more PAINT or remove pixels from your basket.", "error");
      return;
    }
